...
from training import get_minimagen_parser, load_custom_dataset, get_minimagen_dl_opts, \
...
if args.TESTING:
    ...
    train_dataset, valid_dataset = load_custom_dataset(data_path, split_ratio=0.8, valid_num=None)
...
if args.RESTART_DIRECTORY is None:
    imagen_params = dict(
        ...
        cond_drop_prob=0.15,
        #audio_encoder_name=args.T5_NAME
    )
...
print("---before the save training info---")
# Save all training info (config files, model size, etc.)
save_training_info(args, timestamp, unets_params, imagen_params, model_size_MB, training_dir, unets, imagen, train_dataloader)
