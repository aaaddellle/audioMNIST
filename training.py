...
from t5 import custom_encode_text
...
class MinimagenCollator:
    ...
    def __call__(self, batch):
        ...
        #batch = list(filter(lambda x: x['image'] is not None, batch))
         # Filter out instances where the image could not be fetched or is None
        #batch = list(filter(lambda x: x is not None and x['image'] is not None, batch))
        # Filter out instances where the image could not be fetched or is None
        #batch = [x for x in batch if x is not None and x.get('image') is not None]
        # Filter out instances where the image could not be fetched or is None
...
class MinimagenDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #enc = custom_encode_text([self.captions[idx]])
        #audio_embeds = torch.tensor(enc).clone()
        

                
        # Call custom_encode_text function with features, tokenizer, and max_length
        enc, attention_mask = custom_encode_text([self.captions[idx]], tokenizer, max_length)# type: ignore #

        # Convert enc and attention_mask to PyTorch tensors
        enc_tensor = torch.tensor(enc)
        attention_mask_tensor = torch.tensor(attention_mask)

        # Clone the tensors if needed
        enc_tensor_clone = enc_tensor.clone()
        attention_mask_tensor_clone = attention_mask_tensor.clone()

        # You can then use enc_tensor_clone and attention_mask_tensor_clone as needed
        audio_embeds = (enc_tensor_clone, attention_mask_tensor_clone)
        ...
        return {'image': img, 'encoding': audio_embeds}

from PIL import Image
import numpy as np

def load_owndata(data_path):
    images = []
    labels = []
    img_size = (28, 28)
    ...
    for digit in range(10):
        ...
        for filename in os.listdir(digit_path)[:3000]:
...
def MinimagenTrain(timestamp, args, unets, imagen, train_dataloader, valid_dataloader, training_dir, optimizer,
    ...
    def train():
        ...
        for unet_idx in range(len(unets)):
            loss = imagen(images, audio_embeds=encoding, unet_number=unet_idx + 1)
        ...
        if batch_num % args.CHCKPT_NUM == 0:
            ...
            with training_dir("tmp"):
                for idx in range(len(unets)):
                    ...
                    # Print the content before saving
                    print(f"Content of TMP before saving:")
                    print(imagen.unets[idx].state_dict())
            ...
            for vbatch in tqdm(valid_dataloader):
                ...
                for unet_idx in range(len(unets)):
                    running_valid_loss[unet_idx] += imagen(images, audio_embeds=encoding,
                                                           #text_masks=mask,
            ...
            for i, l in enumerate(avg_loss):
                ...
                with training_dir("state_dicts"):
                    ...
                    # Print the content before saving
                    print(f"Content of STATE dict before saving:")
                    print(imagen.unets[i].state_dict())
                    print("best loss")
                    print(best_loss)
        ...
        for batch_num, batch in tqdm(enumerate(train_dataloader)):
            ...
            with training_dir("tmp"):
                for idx in range(len(unets)):
                    ...
                    torch.save(imagen.unets[idx].state_dict(), model_path)
    print(len(unets))
    # Function to ensure directory exists and change into it
    def training_dir(subdir=None):
        training_path = "./training"
        if subdir:
            training_path = os.path.join(training_path, subdir)
...
def save_training_info(args, timestamp, unets_params, imagen_params, model_size, training_dir, unets, imagen, valid_dataloader):
    """
...
def get_default_args(object):
    ...
    if issubclass(object, Unet) and object is not Unet:
        return {**get_default_args(Unet), **object.defaults}
