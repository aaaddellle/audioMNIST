...
class Imagen(nn.Module):
    ...
    def __init__(
        ...
        self.audio_embed_dim = default(audio_embed_dim, lambda: 128)
    ...
    def sample(
        ...
        if exists(audios) and not exists(audio_embeds):
            audio_embeds = custom_encode_text(audios, tokenizer, max_length = 128) # type: ignore
            audio_embeds = map(lambda t: t.to(device), (audio_embeds))

        assert exists(audio_embeds), 'text or text encodings must be passed into Imagen'
        #assert not (exists(audio_embeds) and audio_embeds.shape[-1] != self.audio_embed_dim), f'invalid text embedding dimension being passed in (should be {self.audio_embed_dim})'

        #batch_size = audio_embeds.shape[0]
        #batch_size = audio_embeds['input_ids'].shape[0]
        # Convert map object to a list and stack into a tensor
        audio_embeds_list = list(audio_embeds)
        audio_embeds_tensor = torch.stack(audio_embeds_list, dim=0)

        # Get the batch size from the tensor
        batch_size = audio_embeds_tensor.shape[0]
    ...
    def forward(
        ...
        if exists(audios) and not exists(audio_embeds):
            ...
            audio_embeds = custom_encode_text(audios, name=self.audio_encoder_name)
