...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        self.text_to_cond = nn.Linear(self.audio_embed_dim, cond_dim)
        ...
        self.null_audio_embed = nn.Parameter(torch.randn(1, max_audio_len, cond_dim))
        ...
        self.to_audio_non_attn_cond = nn.Sequential(
    ...
    def _audio_condition(
        ...
        audio_tokens = None
        if exists(audio_embeds):
            
            audio_embeds_dict = dict(audio_embeds)

            for key, value in audio_embeds_dict.items():
                print(f"Key: {key}, Valye: {value}")
            audio_embeds_list = [int(value) for value in audio_embeds.values()]
            audio_embeds_tensor = torch.tensor(audio_embeds_list)
            for element in audio_embeds_tensor:
                print("Element: ", element)


            # Project the text embeddings to the conditioning dimension `cond_dim`.
            audio_tokens = self.audio_to_cond(audio_embeds_tensor)

            # Truncate the tokens to have the maximum number of allotted words.
            audio_tokens = audio_tokens[:, :self.max_audio_len]

            # Pad the text tokens up to self.max_text_len if needed
            audio_tokens_len = audio_tokens.shape[1]
            remainder = self.max_audio_len - audio_tokens_len
            if remainder > 0:
                audio_tokens = F.pad(audio_tokens, (0, 0, 0, remainder))

            # Prob. mask for clf-free guidance conditional dropout. Tells which elts in the batch to keep. Size (b,).
            audio_keep_mask = prob_mask_like((batch_size,), 1 - cond_drop_prob, device=device)

            # Combines T5 and clf-free guidance masks
            audio_keep_mask_embed = rearrange(audio_keep_mask, 'b -> b 1 1')
            ...
            null_audio_embed = self.null_audio_embed.to(audio_tokens.dtype)  # for some reason pytorch AMP not working

            # Replaces masked elements with NULL
            audio_tokens = torch.where(
                audio_keep_mask_embed,
                audio_tokens,
                null_audio_embed
            )
            ...
            mean_pooled_audio_tokens = audio_tokens.mean(dim=-2)

            # Project to `time_cond_dim`
            audio_hiddens = self.to_audio_non_attn_cond(mean_pooled_audio_tokens)  # (b, cond_dim) -> (b, time_cond_dim)

            null_audio_hidden = self.null_audio_hidden.to(t.dtype)

            # Drop relevant conditioning info as demanded by clf-free guidance mask
            audio_keep_mask_hidden = rearrange(audio_keep_mask, 'b -> b 1')
            audio_hiddens = torch.where(
                audio_keep_mask_hidden,
                audio_hiddens,
                null_audio_hidden
            )
        ...
        c = time_tokens if not exists(audio_tokens) else torch.cat((time_tokens, audio_tokens), dim=-2)
